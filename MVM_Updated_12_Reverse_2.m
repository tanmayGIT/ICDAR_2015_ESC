...
% here we have added small JC to the algo  
% this is the good code; updated on 15/06/2015
function [pathCost,pathTarget,indxcol,indxrow,distSum,jumpcost] = MVM_Updated_12_Reverse_2(refSample,testSample,weight,straight)
pathTarget = 1;
    ...
    if(noOfSamplesInTestSample >= noOfSamplesInRefSample)
        ...
        for i = 2:1:noOfSamplesInRefSample
            ...
            for k = stopMotherLeft:1:stopMotherRight
                stopj = min(((k+1+elasticity)-(max(0,(k-(i-1))))),noOfSamplesInTestSample);
                stopLeft  = max(k,1);
...
%                 break;
        ...
        distSum = normalizationTech_6(indxrow,indxcol,distSum,noOfSamplesInRefSample,noOfSamplesInTestSample,jumpcost);      
        return
    end
end
end
