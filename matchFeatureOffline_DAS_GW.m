function resultArr  = matchFeatureOffline_DAS_GW(keepAllFeature,fileNames,refImgName,refImg,technique,normalizeFunc,featureFunc,heightMatter)
imageFilePathRef{1,1} = refImgName;
imageFilePathRef{1,2} = refImg;
nImages = (length(fileNames));

pathstr = '/Users/tanmoymondal/Documents/Study Guru/testOn_GW-DASImage/';
...
imgDir = '/Volumes/Macintosh_HD_2/Word Spotting Dataset/washington_good_set/';
parfor k = 1:(nImages)
    featureMatForImage = keepAllFeature{k,1};
    temImg = imread([imgDir,fileNames{k,1}]);

    testImgRw = size(temImg,1);
    testImgCol = size(temImg,2);
    testImgPath = [imgDir,fileNames{k,1}];
    [~, testName, ~] = fileparts(testImgPath);
    disp(k);
    distVal = wordSpottingBatchOperationComponentImg(featureMatForImage,realIndexMatForImage,...
        0,0,0,0,testImgRw,testImgCol,keepAllInfoRef,featureFunc,normalizeFunc,...
        technique,0,0,heightMatter,testImgPath,imageFiePath_1,testName);
    keepDistAllTest(k,:) = distVal;
    keepName{k,1} = testName;
end

keepDist = keepDistAllTest(:,1);
refName = imageFilePathRef{1,1};
