...
% here in this code we are trying to normalize by total number of links anf
% by total number of inside jumps
function [pathCost,pathTarget,indxcol,indxrow,distSum,jumpcost] = MVM_Updated_12_1(refSample,testSample,weight,straight)
pathTarget = 1;
...
%     end
    ...
    if(noOfSamplesInTestSample >= noOfSamplesInRefSample)
        ...
        while (minrow>=1 && mincol>=1)
            ...
            mincol = mincolTemp;
             if(minrow>=1 && mincol>=1)
                prevRw = Wrapped(1,1);
                prevCol = Wrapped(1,2);
                if( (prevRw ~= minrow) && (prevCol ~= mincol) && ((abs(prevCol - mincol))>1) ) % the many to one and one to many does not work; the other conditon means, you have jumped some elements
                    insideJmpCnt = insideJmpCnt + ((abs(prevCol - mincol))-1); % -1 because, only one difference in column is prefect but not more than that
                end
             end       
        end
        indxrow = Wrapped(:,1);
        ...
        distSum = ((distSum))/(size(indxcol,1)) + insideJmpCnt; % normalize by number of links and by inside jump counter
        return
    end
end
end
