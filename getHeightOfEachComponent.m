function [ComponentHghtWdth1] = getHeightOfEachComponent(Img)
...
% fg = round((length(ComponentHghtWdth2)*60)/100);
% avgWidth = (sum(ComponentHghtWdth2(1:fg,1)))/fg;
% % if(nElement > 1)% so there is more than one character in the word
% %     diff = (ComponentHghtWdth(end,1)) - (ComponentHghtWdth(1,1));
% %     if(diff > 12) % Then there is ascender or descender is present in the word
% %         
% %     end
% % end
