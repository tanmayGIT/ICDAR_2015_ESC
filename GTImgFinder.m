...
%********************************************************************************************
%********************************************************************************************
% this code is made for generating the feature values for ESC datasets with
% GT 


function [GtCorrsRefImgStorer] = GTImgFinder(refImgNm,featureFunc,nomalizeFunc,heightMatter)
% dirOutput = '/Volumes/Macintosh_HD_2/Word Spotting Dataset/Next Collection/ESC-TEST/';
dirOutput12 = '/Volumes/Macintosh_HD_2/Word Spotting Dataset/Dataset_CESR/Grouped_Images_3/Binary_4/';
refImag{1,1} = refImgNm;
disp(refImgNm);
GtCorrsRefImgStorer = cell(length(refImag),3);
for refCnt = 1:1:length(refImag)
%     refGtPath = [dirOutput refImag{1,refCnt}];
    refGtPath12 = [dirOutput12 refImag{1,refCnt}];
    refFolPath =[dirOutput12 refImag{1,refCnt} '/' 'AllComponentImagesGrey' '/'];
    ...
    GtFilePath = [refGtPath12 '/' 'GTCopy_Value.txt'];
    if exist(GtFilePath, 'file') == 2 % then the file exists
        disp(' ');
    else
        GtFilePath = [refGtPath12 '/' 'GT_Value.txt'];
    end
    fid = fopen(GtFilePath,'rt'); % opening the file in read mode
    if fid < 0, error('Cannot open file'); end
    tLine = fgets(fid);
