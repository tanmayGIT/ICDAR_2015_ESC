function resultArr  = matchFeatureOfflineForNewCSER(featureSet,refImgName,refImg,technique,normalizeFunc,featureFunc,heightMatter,mergedGTArr)
imageFilePathRef{1,1} = refImgName;
imageFilePathRef{1,2} = refImg;
nImages = (length(featureSet));

pathstr = '/Users/tanmoymondal/Documents/Study Guru/STUDY GURU/Dataset/ESC/CSER/testOnComponentImage/';
...
jumpCostMat = zeros(nImages,1); 
keepName = cell(nImages,1);
keepAllInfoRef = cell(1,7);
keepAllFeature = featureSet;
...
for eachRefPath = 1:1:1
    ...
    if((exist(imageFiePath_1,'dir'))==0)
        ...
        rmdir(imageFiePath_1,'s');
        mkdir(imageFiePath_1);
    end
    ImgRef = uint8(ImgRef);
    [beforeRLSARef] = wordSpottingBasicOperationRefNoRLSA(ImgRef); % for reference image
    [l1Ref,l4Ref,topLineRef,baseLineRef,componentRefImg,ImgRef] = mainBasicFuncForRef(beforeRLSARef,ImgRef,normalizeFunc,heightMatter);
...
parfor k = 1:(nImages)
    ...
    [distVal,jumpcost] = wordSpottingBatchOperationComponentImg(featureMatForImage,realIndexMatForImage,...
...
for eachRefImgPath = 1:1:1
    ...
    GtArray = mergedGTArr;%getGT_1(refName);
    ...
    for eachIm = 1:1:nImages
        fprintf(fid1,'The Distance Value of %s component is : %d ||| %d \n',keepName{eachIm,1},keepDist(eachIm,1),jumpCostMat(eachIm,1)); %write first value
    end
    fclose(fid1);
    ...
    for divInFol = 1:1:(size(sortedKeepName,1))
        ...
        jumpCostVal = sortedJumpMat(divInFol,1);
        imgIndexOriginal = sortedDistMat(divInFol,2);
        imgName = sortedKeepName{divInFol,1};
        for fdNam  = 1:1:size(GtArray,2)
            [~,namVal,~] = fileparts(GtArray{1,fdNam});
            if( (strcmp(  (namVal),imgName ) ) && (isempty(GtArray {2,fdNam}))  )
        ...
        if((0<= distVal1)&&(distVal1 <= prominentLevel1))
            ...
            myPath = '1st folder';
            fprintf(fid3,'The Distance Value of %d component is : %d ||| %d \n',m1,distVal1,jumpCostVal);
            m1 = m1+1;
        elseif ((prominentLevel1 < distVal1)&&(distVal1 <= prominentLevel2))
            ...
            myPath = '2nd folder';
            fprintf(fid4,'The Distance Value of %d component is : %d ||| %d \n',m2,distVal1,jumpCostVal);
            m2 = m2+1;
        elseif ((prominentLevel2 < distVal1)&& (distVal1 <= prominentLevel3))
            ...
            myPath = '3rd folder';
            fprintf(fid5,'The Distance Value of %d component is : %d  ||| %d \n',m3,distVal1,jumpCostVal);
            m3 = m3+1;
        elseif (prominentLevel3 < distVal1 )
            ...
            myPath = '4th folder';
            fprintf(fid6,'The Distance Value of %d component is : %d  ||| %d \n',m4,distVal1,jumpCostVal);
            m4 = m4+1;
        end
        if(GTmatchFlag == 1)
    ...
    % making the cell formatted GT array into array format GT format
    refinedGTArr = zeros(1,size(GtArray,2));
    dickCnt = 1;
    for iou = 1:1:size(GtArray,2)
        if(~isempty(GtArray{2,iou}))
            refinedGTArr(1,dickCnt) = GtArray{2,iou};
            dickCnt = dickCnt +1;
        end
    end
    
    resultArr = cell((max(refinedGTArr(1,:))),4);
    for genPR = 1:1:max(refinedGTArr(1,:))
        ...
        noRelevantWords = find((refinedGTArr(1,:)) <= genPR);
        calP = (length(noRelevantWords)) / genPR ;
        calR = (length(noRelevantWords)) / size(refinedGTArr,2);
