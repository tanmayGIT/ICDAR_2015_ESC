function [distVal,jumpcost] = performMatchingOnComponentImg_Old(testMat,realIndexTest,componentRefImg,ImgRef,testImgRw,testImgCol,technique,featureFunc,avgWidth,heightMatter,testImgPath,imageFiePath_1,testName)
distVal = -5;
...
[~,distVal,getIndexes,jumpcost] = stringMatchingWithFunction(refMatForMatch,testMat,realIndexRef,realIndexTest,technique,featureFunc,avgWidth);

%     croppedComponent = DrawColorLine(ImgTest,l1,l4,topLineTest,bottomLineTest);
% [pathStrt,namet,extt] = fileparts(testImgPath);
% folNm = pathStrt(1,18:end);
% thisPath = ['/Volumes/Macintosh_HD_2/Word Spotting Dataset/Dataset_CESR/ClusteredComponentImg/componentImg_1/',folNm, '/', namet,extt];
