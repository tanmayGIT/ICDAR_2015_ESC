%This code is same as MVM_Updated_17_2, but here we add 0 as dummy element
% as the 1st row and 1st colum of the arrays. So these 0's are considered
% as dummy and it is assumed that dummy would be matched with dummy then
% the rest of the matching will be continued by normal proceedure
function [pathCost,pathTarget,indxcol,indxrow,distSum,jumpcost] = MVM_Updated_17_4(refSample,testSample,weight,bal)

% adding dummy at the ref and target
nwRef = zeros((size(refSample,1)+2),size(refSample,2));
nwRef(2:end-1,:) = refSample;
refSample = nwRef;

nwTarget = zeros((size(testSample,1)+2),size(testSample,2));
nwTarget(2:end-1,:) = testSample;
testSample = nwTarget;
    ...
    if(noOfSamplesInTestSample >= noOfSamplesInRefSample)
        ...
        for i = 2:1:noOfSamplesInRefSample % so it is actually the 1st row and the other initial row is dummy
            ...
            for k = stopMotherLeft:1:stopMotherRight
                ...
                if( ((i-1) == 1) && (k == 1) ) % only for the top left most first node; to stop many pf query to one of target matching with the dummy node
                    stopLeft  = 2; % the 1st diagonal link
                else
                ...
                for j = (stopLeft):1:stopj
                    if ((j-(k+1)) <= 0)
                        costJump = 0;
                    elseif(i == noOfSamplesInRefSample)
                         costJump = 0;
                    else
                        costJump = jumpcost*(weight*(abs(j-(k+1))));
                    end
                    if(jumpcost~=0)
                        [valMn,indxMn] = min([Dist1(i,j),(jumpcost*2)]); % one added jump cost because we are jumping one element in reference
                    else
                        indxMn = 1;
                        valMn = Dist1(i,j);
                    end
        ...
        mincol = noOfSamplesInTestSample;%(lastElasticity-1)+indices;
        ...
        while (minrow>=1 && mincol>=1)
            ...
            % As jumpMat(minrow,mincol) will always be equal to 1
        ...
        indxrow = indxrow(2:end-1,1); % now reduce the first entry as it will 0 after subtration as 1st dummy element has to be matched with some
        
        indxcol = indxcol - 1;
        indxcol = indxcol(2:end-1,1);
        return
    end
end